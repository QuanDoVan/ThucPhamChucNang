...
exports.getSellingProducts = async () => {
    return await OrderDetail.findAll({
        attributes: [
            'product_id', 'quantity',
            [sequelize.fn('sum', sequelize.col('OrderDetail.quantity')), 'tong']
        ],
        include: [
            {
                model: Product,
        ...
        group: ['product_id'],
        order: [
            [sequelize.fn('SUM', sequelize.col('OrderDetail.quantity')), 'DESC'],
        ]
    })
}
...
exports.getListOrderStatus = async (userId, status) => {
    return await Order.findAll({
        attributes: ['id', 'createTime', 'status', 'total_cart', 'statusPayment', 'payments'],
        include: [
            {
                model: OrderDetail,
                required: false,
