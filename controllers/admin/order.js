...
exports.listOrder = async (req, res, next) => {
    ...
    if (status == "all") {
        ...
        objectStatus.status = constant.STATUS_ORDER.CONFIRM
    } else if (status == "2") {
        objectStatus.status = constant.STATUS_ORDER.CONFIRMED
    } else if (status == "3") {
        objectStatus.status = constant.STATUS_ORDER.DELIVERY
    } else if (status == "4") {
        objectStatus.status = constant.STATUS_ORDER.COMPLETE
    } else if (status == "5") {
        objectStatus.status = constant.STATUS_ORDER.CANCEL
    } else if (status == "6") {
        objectStatus.statusPayment = constant.STATUS_PAYMENT.UNPAID
    } else if (status == "7") {
        objectStatus.statusPayment = constant.STATUS_PAYMENT.PAID
    } 
    const listOrder = await Order.findAll({
        attributes: ['id', 'createTime', 'status', 'payments', 'statusPayment', 'total_cart', 'ship'],
        include: [
            {
                model: User,
                required: false,
...
exports.editOrder = async (req, res, next) => {
    ...
    const detailOrder = await Order.findOne({
        attributes: ['id', 'note', 'createTime', 'status', 'payments', 'statusPayment', 'total_cart', 'ship', 'shipCode', 'shipFee'],
        include: [
            {
                model: User,
                required: false,
